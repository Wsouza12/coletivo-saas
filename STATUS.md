...
- **Customizações Recentes:**
  ...
  - **Limpeza de UI e Dados**: Removidos campos legados de "Prova Social ML" (linkReferencia, posicaoMaisVendido) para deixar o sistema mais limpo.
  - **Geração Rápida de Produtos (Extração)**: Produtos cadastrados a partir da página de um catálogo (pré-cadastro/extracão por IA) agora já nascem "Concluídos" (`isRascunho: false`), sem precisar ir para a fila de rascunhos.
  - **Quantidade Mínima de Compra (`minimoUnidadesPadrao`)**: Implementado nas telas de edição de produto e no pré-cadastro do catálogo, permitindo exigir compras acima de 1 unidade por cliente.
  - **Correção de Rascunhos Presos**: O modal "Concluir Cadastro" (EditarProdutoAtacadoDialog) agora bloqueia o botão de salvar até que as medidas, peso e descrição estejam preenchidos, garantindo que o fluxo não caia em loop e que o produto sempre salve com status ativo (`isRascunho: false`).
