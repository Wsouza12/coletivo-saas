...
import { gerarPromptImagem } from "@/lib/groq";
...
export async function POST(req: Request) {
  ...
  try {
    const prompt = await gerarPromptImagem(parsed.data.ideia);
    return NextResponse.json({ data: { prompt } }, { status: 200 });
  } catch (err) {
    const message = err instanceof Error ? err.message : "Falha ao gerar prompt";
