...
export async function POST(req: Request) {
  ...
  const parsed = categoriaSchema.safeParse(await req.json());
  if (!parsed.success) {
  ...
  const existing = await prisma.categoria.findUnique({ where: { nome: parsed.data.nome } });
  if (existing) {
    return NextResponse.json(
      { error: { code: "NOME_IN_USE", message: "Categoria já existe" } },
      { status: 409 }
    );
  }
