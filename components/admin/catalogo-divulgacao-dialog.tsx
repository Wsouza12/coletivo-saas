              ...
              <div className="flex-shrink-0 w-full md:w-[350px] lg:w-[400px] flex flex-col overflow-y-auto">
                <Tabs defaultValue="whatsapp" className="flex flex-col flex-1">
                  ...
                  <TabsContent value="instagram">
                    <form onSubmit={handlePublicarInstagram} className="flex flex-col gap-4">
                      {/* Status de conexão */}
                      {igStatus === null ? (
                        ...
                        <div className="flex items-center gap-2 rounded-lg border border-green-200 bg-green-50 px-3 py-2 text-sm text-green-800">
                          ...
                          <span>
                            Conectado como <strong>@{igStatus.username}</strong>
                          </span>
                        </div>
                      ) : (
                        <div className="flex flex-col gap-3 rounded-lg border border-orange-200 bg-orange-50 p-3 text-sm text-orange-800">
                          <span className="font-semibold">Instagram não conectado</span>
                          <div className="flex flex-col gap-1.5">
                            <Label className="text-xs text-orange-800">Colar token gerado no portal Meta:</Label>
                            <Textarea
                              value={igTokenManual}
                              onChange={(e) => setIgTokenManual(e.target.value)}
                              rows={3}
                              placeholder="IGAAN..."
                            ...
                            <Button
                              type="button"
                              size="sm"
                              disabled={!igTokenManual.trim() || igSalvandoToken}
                              className="bg-gradient-to-r from-purple-600 to-pink-500 text-white hover:opacity-90"
                              onClick={async () => {
                                setIgSalvandoToken(true);
                                try {
                                  const res = await fetch("/api/instagram/token-manual", {
                                    method: "POST",
                                    headers: { "Content-Type": "application/json" },
                                    body: JSON.stringify({ token: igTokenManual.trim() }),
                                  });
                                  const json = await res.json();
                                  if (!res.ok) throw new Error(json.error?.message ?? "Erro ao salvar token");
                                  toast.success(`Instagram conectado como @${json.data.username}`);
                                  setIgTokenManual("");
                                  carregar();
                                } catch (err: any) {
                                  toast.error(err.message);
                                } finally {
                                  setIgSalvandoToken(false);
                                }
                              }}
                            >
                              {igSalvandoToken ? <Loader2 className="size-3.5 animate-spin" /> : "Salvar Token"}
                            </Button>
                          </div>
                        </div>
                      )}
                        ...
                        <>
                          ...
                          <div className="rounded-lg border border-border bg-muted/30 px-3 py-2 text-sm text-muted-foreground">
                            {paginasSelecionadas.length > 1 ? (
                              <>
                                {igFormato === "STORY"
                                  ? <><strong>{Math.min(paginasSelecionadas.length, 10)} Stories</strong> separados</>
                                  : <><strong>{Math.min(paginasSelecionadas.length, 10)} slides</strong> em carrossel</>
                                }
                                {paginasSelecionadas.length > 10 && <span className="text-orange-500"> (máx 10 — restantes ignoradas)</span>}
                                {" — "} Págs. {paginasSelecionadas.slice(0, 10).join(", ")}
                              </>
                            ) : (
                              <>Será publicada a <strong>página {paginasSelecionadas[0] ?? pagina}</strong> do visualizador.</>
                            )}
                          </div>
                          ...
                          <div className="flex flex-col gap-1.5">
                            <Label>Formato</Label>
                            <Select value={igFormato} onValueChange={(v) => setIgFormato(v as IgFormato)}>
                              <SelectTrigger>
                                <SelectValue />
                              </SelectTrigger>
                              <SelectContent>
                                <SelectItem value="POST">
                                  {paginasSelecionadas.length > 1 ? "Carrossel no Feed (1:1)" : "Post no Feed (1:1)"}
                                </SelectItem>
                                <SelectItem value="STORY">
                                  {paginasSelecionadas.length > 1 ? `${Math.min(paginasSelecionadas.length, 10)} Stories separados (9:16)` : "Story (9:16)"}
                                </SelectItem>
                          ...
                          <div className="flex flex-col gap-1.5">
                            <div className="flex items-center justify-between">
                              <Label>{igFormato === "STORY" ? "Texto do Story" : "Legenda do Post"}</Label>
                              <Button
                                type="button"
                                variant="ghost"
                                size="sm"
                                className="h-7 gap-1 text-xs text-purple-600"
                                onClick={handleGerarConteudoIG}
                                disabled={igGerando}
                              >
                            ...
                            <Textarea
                              ...
                              rows={igFormato === "STORY" ? 3 : 6}
                              placeholder={
                                igFormato === "STORY"
                                  ? "Texto curto e impactante para o story..."
                                  : "Legenda do post com emojis e hashtags..."
                              }
                              className="text-sm"
                            />
                            {igFormato === "POST" && igHashtags && (
                              <p className="text-xs text-muted-foreground truncate" title={igHashtags}>
                                {igHashtags}
                              </p>
                            )}
                          </div>
                          ...
                          <Button
                            type="submit"
                            disabled={igPublicando}
                            className="w-full h-12 mt-2 text-base bg-gradient-to-r from-purple-600 to-pink-500 hover:opacity-90"
                          >
                            {igPublicando ? (
                              <>
                                <Loader2 className="mr-2 size-5 animate-spin" />
                              ...
                              <>
                                <IgIcon className="mr-2 size-5" />
                                {paginasSelecionadas.length > 1
                                  ? igFormato === "STORY"
                                    ? `Publicar ${Math.min(paginasSelecionadas.length, 10)} Stories`
                                    : `Publicar Carrossel (${Math.min(paginasSelecionadas.length, 10)} slides)`
