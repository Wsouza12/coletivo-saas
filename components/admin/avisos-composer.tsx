...
export function AvisosComposer() {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-4xl">
      ...
      {/* ── Instagram ── */}
      <div className="flex flex-col gap-4 rounded-xl border border-pink-200 bg-card p-4">
        <div className="flex items-center gap-2">
          <IgIcon className="size-4 text-pink-500" />
          <span className="font-semibold">Publicar no Instagram</span>
          {igConectado && <span className="ml-auto text-xs text-muted-foreground">@{igUsername}</span>}
        </div>

        {!igConectado ? (
          <p className="text-sm text-muted-foreground">
            Instagram não conectado. Conecte em <strong>Fornecedores → Divulgar Catálogos → aba Instagram</strong>.
          </p>
        ) : (
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div className="flex flex-col gap-3">
                ...
                <div className="flex gap-2">
                  {(["POST", "STORY", "REELS"] as IgFormato[]).map((f) => (
                    <button
                      key={f}
                      type="button"
                      onClick={() => setIgFormato(f)}
                      className={`flex-1 rounded-md border px-2 py-1.5 text-xs font-medium transition-colors ${igFormato === f ? "bg-pink-500 text-white border-pink-500" : "border-border text-muted-foreground hover:border-pink-300"}`}
                    >
              ...
              <InstagramArtePanel
                fotoBlob={arquivo && !arquivo.type.startsWith("video/") ? arquivo : undefined}
                onArte={(blob, legenda, fmt) => {
                  setIgArquivo(new File([blob], "arte.jpg", { type: "image/jpeg" }));
                  setIgLegenda(legenda);
                  setIgFormato(fmt);
                }}
              />

              {/* Imagem manual (alternativa ao gerador) */}
              <div className="flex flex-col gap-1.5">
                <Label className="text-xs">Ou envie imagem diretamente</Label>
                <input ref={igInputRef} type="file" accept="image/*" className="hidden" onChange={(e) => setIgArquivo(e.target.files?.[0] ?? null)} />
                <div className="flex items-center gap-2">
                  <Button type="button" size="sm" variant="secondary" onClick={() => igInputRef.current?.click()}>
                    <Upload className="size-3.5" />
                    {igArquivo ? "Trocar imagem" : arquivo ? "Usar outra" : "Anexar imagem"}
                  </Button>
                  {igArquivo && (
                    <button type="button" onClick={() => setIgArquivo(null)} className="text-xs text-destructive">
                      <X className="inline size-3" /> remover
                    </button>
                  )}
                </div>
                {!igArquivo && arquivo && !arquivo.type.startsWith("video/") && (
                  <button type="button" className="text-xs text-primary hover:underline text-left" onClick={() => setIgArquivo(arquivo)}>
                    ↑ Usar a mesma imagem do WhatsApp
                  </button>
                )}
                {!igArquivo && !arquivo && (
                  <p className="text-xs text-muted-foreground">Anexe uma imagem ou use a mesma do WhatsApp.</p>
                )}
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                <div className="flex items-center justify-between">
                    ...
                    onClick={async () => {
                      ...
                      try {
                        const res = await fetch("/api/instagram/gerar-conteudo", {
                          method: "POST",
                          headers: { "Content-Type": "application/json" },
                          body: JSON.stringify({ nome: texto.slice(0, 80) || "Produto atacado", descricao: texto || null }),
                        });
                        const json = await res.json();
                        if (!res.ok) throw new Error(json.error?.message);
                        const d = json.data;
                        setIgLegenda(igFormato === "POST" ? `${d.legendaPost}\n\n${d.hashtags}` : d.legendaStory);
                  ...
                  >
                    {igGerando ? <Loader2 className="size-3 animate-spin" /> : <Sparkles className="size-3" />}
                    Gerar com IA
                  </button>
                </div>
                <textarea
                  value={igLegenda}
                  onChange={(e) => setIgLegenda(e.target.value)}
                  rows={5}
                  placeholder={igFormato === "POST" ? "Legenda com emojis e hashtags..." : "Texto curto e impactante..."}
                  className="w-full rounded-lg border border-border bg-background p-2.5 text-sm outline-none focus:border-pink-400"
                />
              </div>

              <Button
                type="button"
                disabled={igPublicando || (!igArquivo && (!arquivo || arquivo.type.startsWith("video/")))}
                className="w-full bg-gradient-to-r from-purple-600 to-pink-500 hover:opacity-90 text-white"
                onClick={async () => {
                  const src = igArquivo ?? (arquivo && !arquivo.type.startsWith("video/") ? arquivo : null);
                  if (!src) { toast.error("Anexe uma imagem primeiro"); return; }
                  setIgPublicando(true);
                  try {
                    toast.loading("Preparando imagem...", { id: "ig-av" });
                    const blob = await prepararParaInstagram(src, igFormato);
                    toast.loading("Publicando no Instagram...", { id: "ig-av" });
                    const formData = new FormData();
                    formData.append("imagem", blob, "aviso.jpg");
                    ...
                    toast.success(`Publicado no Instagram! (${igFormato})`, { id: "ig-av" });
                  } catch (err: any) {
                    toast.error(err.message ?? "Falha ao publicar", { id: "ig-av" });
                  } finally {
                    setIgPublicando(false);
                  }
                }}
              >
                {igPublicando ? <Loader2 className="mr-2 size-4 animate-spin" /> : <IgIcon className="mr-2 size-4" />}
                Publicar {igFormato === "POST" ? "no Feed" : igFormato === "STORY" ? "Story" : "Reels"}
              </Button>
            </div>

            {/* Direita: preview */}
            <div className="flex flex-col items-center gap-2">
              <Label className="text-xs self-start">Prévia</Label>
              <div className={`relative overflow-hidden rounded-lg bg-black border border-border ${igFormato === "POST" ? "w-full aspect-square" : "w-40 aspect-[9/16]"}`}>
                {(igPreviewUrl || previewUrl) ? (
                  <img
                    src={igPreviewUrl ?? previewUrl!}
                ...
                <span className="absolute bottom-1 right-1 text-[9px] text-white/70 bg-black/50 rounded px-1">
                  {igFormato === "POST" ? "1:1" : "9:16"}
                </span>
              </div>
              {igLegenda && (
                <p className="text-xs text-muted-foreground line-clamp-3 w-full">{igLegenda}</p>
              )}
            </div>
          </div>
        )}
