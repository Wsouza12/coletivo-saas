          ...
          {catalogo ? (
            <div className="flex-1 flex overflow-hidden">
              ...
              <div className="w-[40%] flex flex-col bg-card overflow-y-auto">
                ...
                {/* ── Instagram ── */}
                <div className="px-6 pb-6 flex flex-col gap-3 border-t pt-5">
                  <h3 className="font-semibold flex items-center gap-2">
                  ...
                  {igConectado ? (
                    <>
                      ...
                      <div className="flex gap-2">
                        <button
                          type="button"
                          onClick={() => setIgFonte("recorte")}
                          className={`flex-1 rounded-md border px-3 py-1.5 text-xs font-medium transition-colors ${igFonte === "recorte" ? "bg-slate-700 text-white border-slate-700" : "border-border text-muted-foreground"}`}
                        >
                          Recorte
                        </button>
                        <button
                          type="button"
                          onClick={() => setIgFonte("pagina")}
                          className={`flex-1 rounded-md border px-3 py-1.5 text-xs font-medium transition-colors ${igFonte === "pagina" ? "bg-slate-700 text-white border-slate-700" : "border-border text-muted-foreground"}`}
                        >
                          Página {pagina}
                        </button>
                      </div>
                      {igFonte === "recorte" && !fotoBlob && (
                        <p className="text-xs text-orange-600">Arraste um recorte no PDF para usar esta opção.</p>
                      )}
                      {igFonte === "pagina" && (
                        <button
                          type="button"
                          ...
                          onClick={async () => {
                            ...
                            if (blob) setIgPreviewUrl(URL.createObjectURL(blob));
                          }}
                        >
                          Pré-visualizar página {pagina}
                        </button>
                      )}
                      ...
                      <div className="flex gap-2">
                        {(["POST", "STORY", "REELS"] as IgFormato[]).map((f) => (
                          <button
                            ...
                            className={`flex-1 rounded-md border px-2 py-1.5 text-xs font-medium transition-colors ${igFormato === f ? "bg-pink-500 text-white border-pink-500" : "border-border text-muted-foreground"}`}
                          >
                            {f === "POST" ? "Post" : f === "STORY" ? "Story" : "Reels"}
                          </button>
                        ))}
                      </div>
                      ...
                      {/* Preview */}
                      {igPreviewUrl && (
                        <div className={`relative mx-auto overflow-hidden rounded-lg bg-black ${igFormato === "POST" ? "w-40 h-40" : "w-24 h-40"}`}>
                          <img src={igPreviewUrl} className="w-full h-full object-contain" alt="preview" />
                            ...
                            onClick={async () => {
                              ...
                              try {
                                const res = await fetch("/api/instagram/gerar-conteudo", {
                                  method: "POST",
                                  headers: { "Content-Type": "application/json" },
                                  body: JSON.stringify({ nome: nomeProduto || nome, categoria: categoria || null }),
                                });
                                const json = await res.json();
                                if (!res.ok) throw new Error(json.error?.message);
                                const d = json.data;
                                setIgLegenda(igFormato === "STORY" ? d.legendaStory : `${d.legendaPost}\n\n${d.hashtags}`);
                      ...
                      <Button
                        type="button"
                        disabled={igPublicando || (!igArquivo && igFonte === "recorte" && !fotoBlob)}
                        className="w-full bg-gradient-to-r from-purple-600 to-pink-500 hover:opacity-90 text-white"
                        onClick={async () => {
                          // Arte gerada tem prioridade sobre recorte/página
                          let rawBlob: Blob | null = igArquivo ?? null;
                          if (!rawBlob) {
                            if (igFonte === "recorte") {
                              if (!fotoBlob) { toast.error("Recorte uma imagem ou gere uma arte primeiro"); return; }
                              rawBlob = fotoBlob;
                            } else {
                              toast.loading(`Extraindo página ${pagina}...`, { id: "ig-bc" });
                              rawBlob = await pdfViewerRef.current!.extrairPagina(pagina);
                            }
                          }
                          setIgPublicando(true);
                          try {
                            // Arte gerada já está no formato correto, não redimensionar novamente
                            const blob = igArquivo ? rawBlob! : await prepararParaInstagram(rawBlob!, igFormato);
                            // Atualiza preview
                            const url = URL.createObjectURL(blob);
                            setIgPreviewUrl(url);

                            toast.loading("Publicando no Instagram...", { id: "ig-bc" });
                            const formData = new FormData();
                            ...
                            toast.success(`Publicado no Instagram! (${igFormato})`, { id: "ig-bc" });
                            limparParaProximoProduto();
                          } catch (err: any) {
                            toast.error(err.message ?? "Falha ao publicar", { id: "ig-bc" });
                          } finally {
                      ...
                      >
                        ...
                        Publicar {igFormato === "STORY" ? "Story" : "no Feed"}
                      </Button>
                    </>
                  ) : (
                    <p className="text-xs text-muted-foreground">
                      Instagram não conectado. Conecte em <strong>Divulgar Catálogos → aba Instagram</strong>.
                    </p>
                  )}
                </div>
              </div>
